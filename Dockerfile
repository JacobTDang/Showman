...
RUN set -eux; \
  ...
  ok=1; \
  apt-get -o Acquire::Check-Valid-Until=false -o Acquire::Retries=2 -o Acquire::http::Timeout=20 update || ok=0; \
  if [ "$ok" = 1 ]; then \
    apt-get -o Acquire::Retries=2 -o Acquire::http::Timeout=20 install -y --no-install-recommends \
