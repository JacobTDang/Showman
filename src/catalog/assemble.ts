...
function assembleNodeLevel(registry: BuilderRegistry, req: AssembleRequest): SceneSpec {
  ...
  const narration = narrationTrack(req.beat?.narrationBeats, duration, req.voice);
  if (narration) spec.narration = narration;
  return spec;
}
...
function narrationTrack(beats: string[] | undefined, duration: number, voice?: string): SceneSpec["narration"] {
  const lines = (beats ?? []).map((b) => b.trim()).filter(Boolean);
  if (lines.length === 0) return undefined;
  const slot = duration / lines.length;
  const segments: NarrationSegment[] = lines.map((text, i) => ({
    t: Number((i * slot).toFixed(3)),
    text,
    duration: Number((slot * 0.9).toFixed(3)),
  }));
  return { segments, ...(voice ? { voice } : {}) };
}
